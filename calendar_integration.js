...
function getTravelMinutesByCalendarColor(userId, color) {
  ...
  const minutes = getTagByColor(userId, color).minutes;
...
function checkAndNotify() {
  ...
  for (let row = 2; row <= lastRow; row++) {
    ...
    const tagName = getTagByColor(userId, sheet.getRange(row, 5).getValue().toString()).name;
    ...
    if (departureTime <= now && now <= eventStartTime) {
      const moStatus = getUser(userId).remoToken ? getMotionStatus(userId) : 0;

      if (moStatus == 1 || !getUser(userId).remoToken) {
        sendTextMessage(
          userId,
          [
            "出発時間です！",
